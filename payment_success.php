    ...
    response.then((v)=>{
       ...
       ihtml+= `<h1>Paid Successfully</h1>
            <div class="success_container">
                <h2>Amount</h2>
                <div class="amount">
                <h2>Rs ${wallet[n-1].amount}</h2><i class="fa-solid fa-circle-check" style="color: #8cb62f;background-color: white;border-radius: 100%;"></i>
                </div>
                <p>Payment Id : ${wallet[n-1].transaction_id}</p>
                <p>${wallet[n-1].date}, ${wallet[n-1].time}</p>
                <p>Paid using ${wallet[n-1].transaction_mode}</p>
            </div>
            <div class="click">
